...
sf::Texture* AssetManager::loadTexture(const char* textureDir)
{
    ...
    if (texture.loadFromFile(textureDir))
    {
        ...
        LOG_INFO("assetMan/AssetManager.cpp", std::format("Loaded texture from directory=({})", textureDir));
    ...
    LOG_WARN("assetMan/AssetManager.cpp", std::format("Could not find texture with directory=({}) using placeholder texture", textureDir));
    ...
    LOG_ERROR("assetMan/AssetManager.cpp", std::format("Could not find external texture in directory=({}), returning blank texture", externalTextureDir));
...
sf::Font* AssetManager::loadFont(const char* fontDir)
{
    ...
    if (font.loadFromFile(fontDir))
    {
        ...
        LOG_INFO("assetMan/AssetManager.cpp", std::format("Loaded font from directory=({})", fontDir));
    ...
    LOG_WARN("assetMan/AssetManager.cpp", std::format("Could not find font with directory=({}) using placeholder font", fontDir));
    ...
    LOG_ERROR("assetMan/AssetManager.cpp", std::format("Could not find external font in directory=({}), returning nullptr", externalFontDir));
    return nullptr;
}
...
sf::Texture* AssetManager::findCachedTexture(const char* uniqueKey)
{
    ...
    {
        LOG_INFO("assetMan/AssetManager.cpp", std::format("Found cached texture with directory=({})", uniqueKey));
        return &(it->second);
    }
    return nullptr;
}
...
sf::Font* AssetManager::findCachedFont(const char* uniqueKey)
{
    ...
    {
        LOG_INFO("assetMan/AssetManager.cpp", std::format("Found cached font with directory=({})", uniqueKey));
        return &(it->second);
    }
    return nullptr;
}
