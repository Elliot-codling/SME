...
sf::Vector2f Text::getPosition() const
{
    ...
    LOG_TRACE("text/Text.cpp", fmt::format("'getPosition()' requested (ID={}) (pos={},{})", getId(), position.x, position.y));
    return position;
}
...
void Text::setPosition(const sf::Vector2f position)
{
    ...
    LOG_TRACE("text/Text.cpp", fmt::format("'setPosition()' requested (ID={}) (pos_requested={},{}) (pos={},{})", getId(), position.x, position.y, newPosition.x, newPosition.y));
...
void Text::incrementPosition(const sf::Vector2f position)
{
    ...
    LOG_TRACE("text/Text.cpp", fmt::format("'incrementPosition()' requested (ID={}) (pos={},{})", getId(), newPosition.x, newPosition.y));
    m_text.setPosition(newPosition);
}
void Text::setOrigin(const sf::Vector2f origin)
{
    LOG_TRACE("text/Text.cpp", fmt::format("'setOrigin()' requested (ID={}) (origin={},{})", getId(), origin.x, origin.y));
    m_text.setOrigin(origin.x, origin.y);
}
void Text::setOffset(const sf::Vector2f offset)
{
    LOG_TRACE("text/Text.cpp", fmt::format("'setOffset()' requested (ID={}) (offset={},{})", getId(), offset.x, offset.y));
...
{
    LOG_TRACE("text/Text.cpp", fmt::format("'setAngle()' requested (ID={}) (angle={})", getId(), angle));
    m_text.setRotation(angle);
}
void Text::incrementAngle(const float angle)
{
    LOG_TRACE("text/Text.cpp", fmt::format("'incrementAngle()' requested (ID={}) (angle={})", getId(), angle));
    m_text.rotate(angle);
}
...
sf::Vector2f Text::getSize() const
{
    ...
    LOG_TRACE("text/Text.cpp", fmt::format("'getSize()' requested (ID={}) (size={},{})", getId(), size.x, size.y));
    return size;
}
void Text::setFontSize(const uint8_t fontSize)
{
    LOG_TRACE("text/Text.cpp", fmt::format("'setFontSize()' requested (ID={}) (size={})", getId(), fontSize));
